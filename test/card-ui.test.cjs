...
test('does not render a Shown because row on any replacement card', () => {
  assert.doesNotMatch(contentSource, /Shown because/);
  assert.doesNotMatch(
    contentSource,
    /className = 'pangram-gallery-card__source-actor'/,
    'replacement cards should not create a feed-source display row'
  );
});
...
test('captures the feed-source actor for local history without rendering it on the card', () => {
  ...
  assert.match(contentSource, /sourceActor:\s*card\.pangramGallerySourceActor/);
  assert.doesNotMatch(contentSource, /pangram-gallery-card__source-actor/);
  assert.doesNotMatch(contentSource, /pangram-gallery-source-popover/);
  assert.match(
    contentCss,
    /\.pangram-gallery-card__author:focus-visible,\s*\.pangram-gallery-card__poem:focus-visible,\s*\.pangram-gallery-card__toggle:focus-visible\s*\{[^}]*outline:/s
  );
});
