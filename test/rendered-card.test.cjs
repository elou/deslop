...
test('keeps a landscape artwork gallery-bounded in a LinkedIn-width rendered feed', () => {
  ...
  assert.equal(poem.footerDisplay, 'flex', 'poetry should retain the shared metadata footer');
  assert.ok(poem.footerHeight > 0, 'poetry footer should remain laid out');
});
